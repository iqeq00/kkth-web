...
layui.use(['config', 'lichee', 'jquery', 'layer', 'table', 'form', 'laydate', 'imageUpload', 'fileUpload'], function () {
    ...
    var tableInfo = table.render({
        ...
        cols: [[
            ...
            {field: 'itemnumber', align: 'center', sort: true, title: '作品编号'},
            {field: 'scienceitem', align: 'center', sort: true, title: '作品名称'},
            {field: 'masses', align: 'center', sort: true, title: '受众群体'},
    ...
    table.on('tool(tableFilter)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('确定要删除吗？', function (index) {
                layer.load(2);
                lichee.delete('/sciencework/' + obj.data.id, {}, function () {
    ...
    var showEditModel = function (data) {
        layer.open({
            ...
            success: function () {
                ...
                imageUpload.init("uploadBtn", "img", "sciencework");
                fileUpload.init("uploadFile", "url", "sciencework");
                if (data) {
    ...
    form.on('submit(formSubmit)', function (data) {
        layer.load(2);
        if (data.field.id) {
            lichee.put('/sciencework/' + data.field.id, {data: data.field}, function (res) {
                callFunction(res);
            });
        } else {
            lichee.post('/sciencework', {data: data.field}, function (res) {
                callFunction(res);
            });
        }
        return false;
