layui.use(['config', 'lichee', 'jquery', 'layer', 'table', 'form', 'imageUpload'], function () {
    ...
    var tableInfo = table.render({
        ...
        cols: [[
            ...
            {field: 'protectionObjectNumber', align: 'center', sort: true, title: '保护对象编号'},
            {field: 'name', align: 'center', sort: true, title: '名称'},
            {field: 'protectionObjectCategory', align: 'center', sort: true, title: '保护对象类型'},
            {field: 'geologicalAge', align: 'center', sort: true, title: '地质年代'},
            // {field: 'safeguard', align: 'center', sort: true, title: '保护措施'},
            {field: 'lengths', align: 'center', sort: true, title: '长度'},
            {field: 'acreage', align: 'center', sort: true, title: '面积'},
    ...
    table.on('tool(tableFilter)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('确定要删除吗？', function (index) {
                layer.load(2);
                lichee.delete('/majorProtection/' + obj.data.id, {}, function () {
    ...
    var showEditModel = function (data) {
        layer.open({
            ...
            success: function () {
                ...
                imageUpload.init("uploadBtn", "img", "majorProtection");
                if (data) {
    ...
    form.on('submit(formSubmit)', function (data) {
        layer.load(2);
        if (data.field.id) {
            lichee.put('/majorProtection/' + data.field.id, {data: data.field}, function (res) {
                callFunction(res);
            });
        } else {
            lichee.post('/majorProtection', {data: data.field}, function (res) {
                callFunction(res);
            });
        }
        return false;
